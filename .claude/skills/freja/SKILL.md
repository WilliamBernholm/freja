...
- A **lecture** (`Lectures/NN_Claude_*.md`) — e.g. "quiz me on lecture 6", "quiz me on DMD".
- A **course textbook chapter** (any `Literature/Chapter_N_*.md` — there is
  more than one; always `glob` the folder rather than assuming which
  chapters exist) — e.g. "quiz me on the SVD chapter", "quiz me on
  Fourier transforms", "quiz me on chapter 3".
- A **homework problem** (`Homeworks/**`, PDFs, plus anything under `Homeworks/Generated/` or any `Homeworks/<Domain>/` subfolder) — e.g. "quiz me on HW1 problem 6".
...
- **Notation consistency is a hard rule, not just "grounding."** Reuse the
  *exact* symbols/variable names the resolved source content actually
  ...
  2026-07-09, a generated rocket-propulsion problem used "effective
  exhaust velocity $c$" and "mass ratio $R = m_0/m_f$" from the
...
- If only a topic keyword is given (e.g. "DMD", "SINDy", "compressed
  sensing"), use `grep` across `Lectures/*.md` to find the best-matching
  file and section. Some lecture files cover more than one topic (e.g.
  `06_Claude_robust_pca_and_dmd.md` covers *both* Robust PCA and DMD) — if
...
- Use `glob` to find the PDF — check both `Homeworks/<HW>/<HW>.pdf` (e.g.
  ...
  `Homeworks/*/` broadly for generated problems, which use markdown
  instead of PDF — this includes `Homeworks/Generated/` and any
  `Homeworks/<Domain>/` subfolder (e.g. `Homeworks/Aerospace/`, or
  whatever other domain/prefix your teacher chose — never assume
  Aerospace is the only one, or the only possible one).
...
- **First, check for a Freja-safe companion file** — `glob` in the same
  ...
  `Homeworks/Aerospace/P01_relative-motion_freja.md`; or
  `Homeworks/Generated/HW<N>_generated_<slug>_freja.md`).
  This file, when it exists, is purpose-built practice material:
  ...
  - **If not found**: fall through to the `solutions/` lookup below,
    unchanged — this is the known, accepted gap for material produced
    before this mechanism existed.
- Look for a matching solution file in the sibling `solutions/` directory
  (e.g. `Homeworks/solutions/HW1_solutions.*`, or
  `Homeworks/<Domain>/solutions/...` for any domain subfolder)
