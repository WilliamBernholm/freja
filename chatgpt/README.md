...
the right thing before you start.

## The one rule that matters

**Upload one section at a time, not a whole textbook chapter.**

ChatGPT does not put large files into the conversation. Above a certain size
it indexes the file and searches it, which means it only ever sees fragments
and fills the gaps from what it already knows about the subject. That is
exactly when it starts sounding confident about things your course never
said.
